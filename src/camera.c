...
void camera_init(Camera *camera, int width, int height) {
    ...
    camera->rendering_depth = 20;
    camera->width = width;
    camera->height = height;
    camera->aspect_ratio = (float)width / (float)height;
    camera->vfov = 50.f;
    // camera->lookfrom = vec3_create(-0.4, -0.4, 2);
    // camera->lookfrom = vec3_create(-1.6, -.4, 2);
    camera->lookfrom = vec3_create(-1.6, -.4, 2);
    ...
    Vec3 upper_left_corner = vec3_sub(
        vec3_sub(camera->position, vec3_mul(camera->w, focal_length)),
        vec3_sub(vec3_mul(viewport_u, 0.5f), vec3_mul(viewport_v, 0.5f))
    );
    Vec3 pixel_upper_left = vec3_add(
        upper_left_corner,
        vec3_mul(vec3_sub(camera->pixel_delta_u, camera->pixel_delta_v), 0.5f)
    );
...
void camera_lookat(Vec3 eye, Vec3 center, Vec3 up, Matrix44 *m) {
    ...
    m->m11 = right.x, m->m12 = right.y; m->m13 = right.z, m->m14 = 0;
    m->m21 = new_up.x, m->m22 = new_up.y; m->m23 = new_up.z, m->m24 = 0;
    m->m31 = forward.x, m->m32 = forward.y; m->m33 = forward.z, m->m34 = 0;
    m->m41 = eye.x, m->m42 = eye.y; m->m43 = eye.z, m->m44 = 1;
}

Vec3 camera_pixel_sample(Camera *camera) {
    float px = -0.5f + rand_range(0, 1);
    float py = -0.5f + rand_range(0, 1);
    Vec3 sample =  vec3_add(
        vec3_mul(camera->pixel_delta_u, px),
        vec3_mul(camera->pixel_delta_v, py)
    );
    return sample;
}
...
void camera_ray_from_pixel(Camera *camera, int i, int j, Ray *ray) {
    Vec3 pixel_center = vec3_add(
        camera->pixel_upper_left,
        vec3_sub(vec3_mul(camera->pixel_delta_u, i), vec3_mul(camera->pixel_delta_v, j))
    );
    ...
    ray->direction = vec3_norm(ray_direction);
}
