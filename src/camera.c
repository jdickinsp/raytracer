...
void camera_lookat(Vec3 eye, Vec3 center, Vec3 up, Matrix44 *m) {
    ...
    m->m11 = right.x, m->m12 = right.y;
    m->m13 = right.z, m->m14 = 0;
    m->m21 = new_up.x, m->m22 = new_up.y;
    m->m23 = new_up.z, m->m24 = 0;
    m->m31 = forward.x, m->m32 = forward.y;
    m->m33 = forward.z, m->m34 = 0;
    m->m41 = eye.x, m->m42 = eye.y;
    m->m43 = eye.z, m->m44 = 1;
}
...
void camera_ray_from_pixel(Camera *camera, int i, int j, Ray *ray) {
    Vec3 pixel_center = vec3_add(camera->pixel_upper_left,
