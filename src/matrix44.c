...
void matrix44_debug_print(Matrix44 *m) {
    printf("[[%f %f %f %f]\n[%f %f %f %f]\n[%f %f %f %f]\n[%f %f %f %f]]\n", m->m11, m->m12, m->m13, m->m14, m->m21,
           m->m22, m->m23, m->m24, m->m31, m->m32, m->m33, m->m34, m->m41, m->m42, m->m43, m->m44);
}
...
Matrix44 matrix44_inverse(Matrix44 *m) {
    ...
    float determinant =
        m->m11 * without_row_1 - m->m21 * without_row_2 + m->m31 * without_row_3 - m->m41 * without_row_4;
