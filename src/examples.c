...
Scene *create_scene_rand_spheres() {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        Material *rand_material = material_create(rand_range(0, .9), rand_range(0, .9), rand_range(0, .4), reflective, rand_color, ior);
...
Scene *create_scene_with_obj_file() {
    ...
    for (int n=0; n < mesh->vertex_count / 3; n++) {
        ...
        Triangle *triangle1 = triangle_create_with_normals(
            mesh->vertices[i], mesh->vertices[j], mesh->vertices[k], t_material1,
